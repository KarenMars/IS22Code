...
FEATS_DIR=../../feats
# model name
MODEL_NAME=wav2vec1-base-ls960
# checkpoint of the pretrained model 
W2V1_MODEL=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/hangji/open_source_ckpt/fairseq/wav2vec_large.pt
...
if [ $stage -le 0 ]; then
    for corpus in timit ; do
        echo "Extracting wav2vec 1.0 features for ${corpus}..."
        export CUDA_VISIBLE_DEVICES=1
        gen_wav2vec1_feats.py \
            --use_gpu \
            --disable-progress \
	        $W2V1_MODEL $FEATS_DIR/$corpus/${MODEL_NAME} \
